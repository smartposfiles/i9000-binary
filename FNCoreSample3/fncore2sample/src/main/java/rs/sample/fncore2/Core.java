...
public class Core extends AppCore implements Application.ActivityLifecycleCallbacks {
    ...
    public ListAdapter getLogger() {
        return LOGGER;
    }

    public int checkMarkingItem(SellItem item) throws RemoteException {
        return getStorage().checkMarkingItem(item);
    }

    public int confirmFailedMarkingItem(SellItem item, boolean accepted) throws RemoteException {
        return getStorage().confirmFailedMarkingItem(item, accepted);
    }
